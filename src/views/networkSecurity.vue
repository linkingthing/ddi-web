                    ...
                    <table class="table-default">
                        ...
                        <tbody>
                            <tr v-for="item in this.resList" :key="item.id">
                                <td v-if="item.ip== undefined || item.ip.length <= 0"><p>-</p></td>
                                <td v-else><p v-for="value in item.ip" :key="value.id" >{{value}}</p></td>
...
export default {
   ...
   components: {
    ...
    goConfig(){
     ...
     getView(){
        this.$axios.get('http://10.0.0.19:8081/apis/linkingthing.com/example/v1/forwards', {
          })
          .then(res=> {
        ...
        delect(data){
            this.$Modal.confirm({
                ...
                content: '确定删除？',
               onOk: () => {
            this.$axios.delete('http://10.0.0.19:8081/apis/linkingthing.com/example/v1/forwards/123',{
        } )
        .then(res=>{
